...
| `sm_zr_repeat_kill`         | 1    | 禁用在地图重复死亡下的玩家复活。设置为`0` 可解决机制误触发下无法复活.可使用`!rk`强制关闭核爆检测  |
| `zr_zombie_dropknife_when_knife_item`   | 1    | 启用或禁用僵尸丢弃匕首                        |
...
| 名称                    | 值 | 说明 |
|-------------------------|--------|------|
| `hook_boss_money_min` | 4      |      |
| `hook_boss_money_max` | 12     |      |
